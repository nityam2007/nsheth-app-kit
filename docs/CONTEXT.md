...
2. Current version: `0.4.0`. The TanStack, copied Untitled UI, identity, admin, content/blog, and product catalogue/RFQ foundations are complete and verified. The admin shell has grouped permission-filtered navigation, a persistent desktop sidebar, and a native-dialog mobile drawer; resources include responsive list, create, detail, edit, and delete workflows. Compose provides PostgreSQL; Node/npm run on the host.
3. Service booking is implemented at `/services` and `/admin/services`, with requests at `/admin/bookings`. Hospitality is at `/stays`, `/admin/properties`, and `/admin/reservations`. Next: commerce, production identity/operations, deployment targets, and reusable compositions.
