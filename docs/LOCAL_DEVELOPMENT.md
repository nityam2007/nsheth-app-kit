...
0.9.2 disables automatic dependency discovery after startup. Add new CommonJS client dependencies to `optimizeDeps.include`. Use `npm run test:navigation` against your running server to test clicks and transitions in one client session, in addition to `test:hydration` for fresh document loads. The route-error recovery button reloads the document instead of reusing a potentially stale module graph. See [navigation follow-up](NAVIGATION_2026-09-09.md).

If an existing tab still reports a null React hook after a development update, expand the error details. Different `?v=` values on `react.js` and `react-dom_client.js` identify mixed dependency graphs. Use Ctrl+Shift+R once to discard the older graph. If a freshly reloaded tab still fails, retain the full new stack rather than resetting the database. The creation-form regression is included in `npm run test:navigation`.
