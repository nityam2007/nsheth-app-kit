...
try{
  ...
  const article={title:'Integration article',slug:'article-'+suffix,excerpt:'Editorial summary',body:'## Heading\n\n<script>alert(1)</script>',status:'PUBLISHED',publishedAt:'2030-01-01T00:00:00Z',author:'Test author',tags:['Testing']}
  assert.equal((await call('createAdminPost',article,cookie)).status,200)
  const post=await db.post.findUniqueOrThrow({where:{slug:article.slug}})
  const scheduled=await server.fetch(new Request(new URL('/blog/'+article.slug,origin)));await scheduled.text();assert.equal(scheduled.status,404,'Scheduled article remains private')
  const edit={...article,currentSlug:article.slug,expectedVersion:post.version,publishedAt:'2026-01-01T00:00:00Z'}
  assert.equal((await call('updateAdminPost',edit,cookie)).status,200)
  assert.equal((await call('updateAdminPost',edit,cookie)).status,409,'Stale editorial save rejected')
  const published=await server.fetch(new Request(new URL('/blog/'+article.slug,origin))),markup=await published.text()
  assert.equal(published.status,200);assert.ok(markup.includes('&lt;script&gt;'),'Article HTML is escaped')
  const revision=await db.postRevision.findFirstOrThrow({where:{postId:post.id}})
  assert.equal((await call('restorePostRevision',{postId:post.id,revisionId:revision.id,expectedVersion:2},cookie)).status,200)
  assert.equal((await db.post.findUniqueOrThrow({where:{id:post.id}})).status,'DRAFT','Restoration cannot unexpectedly republish')
  const enquiry=await db.enquiry.create({data:{productId:product.id,name:'Test',email,quantity:1,message:'Please send a quote'}})
  const triage={id:enquiry.id,status:'IN_PROGRESS',expectedVersion:enquiry.version,ownership:'claim',followUpAt:'2030-01-01T00:00:00Z',note:'Preparing a detailed quote'}
  ...
  assert.equal((await db.enquiry.findUniqueOrThrow({where:{id:enquiry.id}})).assigneeId,user.id)
  const other=await db.user.create({data:{email:'other-'+email}})
  const otherSession=await db.session.create({data:{userId:other.id,tokenHash:await hashSessionToken(createSessionToken()),expiresAt:new Date(Date.now()+600000)}})
  assert.equal((await call('revokeOwnSession',{id:otherSession.id},cookie)).status,404,'Cannot revoke another account session')
  ...
  const crossDemotions=await Promise.all([call('updateAccess',{userId:peer.id,role:'customer',disabled:false},cookie),call('updateAccess',{userId:user.id,role:'customer',disabled:false},`__Host-session=${peerToken}`)])
  assert.equal(crossDemotions.filter(response=>response.status===200).length,1,'Only one conflicting admin change succeeds')
  assert.ok(crossDemotions.every(response=>[200,401,403].includes(response.status)))
  await db.auditEvent.deleteMany({where:{entityType:'access',entityId:{in:[user.id,peer.id]}}})
  ...
  const fixtureEnquiries=await db.enquiry.findMany({where:{email},select:{id:true}})
  await db.auditEvent.deleteMany({where:{entityId:{in:fixtureEnquiries.map(row=>row.id)}}})
