...
try {
  ...
  assert.equal(
    await db.bookingRequest.count({ where: { slotId: slot.id } }),
    1,
  );
  const stay = {
    key: attempt(),
  ...
  // Render actual production pages in-process; no listener or browser is started.
  const pages = new Set([
    "/",
    "/login",
    "/register",
    ...
    "/account/profile",
    "/account/security",
    "/admin/bookings?status=REQUESTED",
    "/admin/products?status=DRAFT",
  ]);
  let rendered = 0;
  for (const path of pages) {
    assert.ok(pages.size <= 150, "Page sweep must remain bounded");
    const response = await server.fetch(
      new Request(new URL(path, origin), { headers: { cookie } }),
    );
    const html = await response.text();
    assert.equal(response.status, 200, `Page renders: ${path}`);
    assert.equal(
    ...
    assert.equal(
      ...
      1,
      `One page heading: ${path}`,
    );
    assert.doesNotMatch(
      html,
      /Something needs attention|Unable to load this page|Cannot read properties of null/,
      path,
    );
    ...
    for (const match of main.matchAll(/href="([^"#]+)"/g)) {
      ...
      if (
        ...
        /^\/(admin|account|blog|services|stays)(\/|$)/.test(target.pathname)
      )
        pages.add(target.pathname + target.search);
    }
    rendered++;
  }
  console.log(
    `Production page sweep: ${rendered} pages; one main and one heading each.`,
  );
